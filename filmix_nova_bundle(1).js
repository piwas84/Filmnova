  ...
  function FilmixSource(component, object) {
    ...
    this.search = function (obj, data) {
      ...
      net = request(url, function (json) {
        ...
        var card = null;
        for (var j = 0; j < cards.length; j++) if (cards[j].year === year) { card = cards[j]; break; }
        if (!card && orig) for (var k = 0; k < cards.length; k++) if (cards[k].original_title === orig) { card = cards[k]; break; }
        if (!card && cards.length === 1) card = cards[0];

        if (card && card.id) self.find(card.id);
        else {
          wait_similars = true;
    ...
    function extractData(data) {
      ...
      if (pl.playlist && typeof pl.playlist === 'object' && Object.keys(pl.playlist).length) {
        var seas_num = 0;
        for (var season in pl.playlist) {
          if (!Object.prototype.hasOwnProperty.call(pl.playlist, season)) continue;
          var episode = pl.playlist[season];
          seas_num++;
          var transl_id = 0;
          for (var voice in episode) {
            if (!Object.prototype.hasOwnProperty.call(episode, voice)) continue;
            transl_id++;
            var episode_voice = episode[voice];
            var items = [];
            for (var ID in episode_voice) {
              if (!Object.prototype.hasOwnProperty.call(episode_voice, ID)) continue;
              var fe = episode_voice[ID];
              if (!fe || !fe.link) continue;
              var quality_eps = [];
              if (fe.qualities && fe.qualities.length) {
                for (var qi = 0; qi < fe.qualities.length; qi++)
                  if (fe.qualities[qi] <= maxQ) quality_eps.push(fe.qualities[qi]);
              }
              if (!quality_eps.length) quality_eps = [720, 480];
              var max_quality = Math.max.apply(null, quality_eps);
              var stream_url = String(fe.link).replace('%s.mp4', max_quality + '.mp4');
              var seas_n = seas_num, epis_n = parseInt(ID, 10) || 1;
              try {
                var m = stream_url.match(/s(\d+)e(\d+)/i);
                if (m) { seas_n = parseInt(m[1], 10); epis_n = parseInt(m[2], 10); }
              } catch (e) {}
              items.push({
                translation: transl_id, season: seas_n, episode: epis_n,
                title: voice + ' — S' + seas_n + 'E' + epis_n,
                quality: max_quality, qualities: quality_eps, link: stream_url, voice: voice
              });
            }
            if (!extract[voice]) extract[voice] = { json: {}, translation: transl_id };
            extract[voice].json[seas_num] = { id: seas_num, folder: items };
          }
        }
      } else if (pl.movie && typeof pl.movie === 'object') {
        var tid = 0;
        for (var mid in pl.movie) {
          if (!Object.prototype.hasOwnProperty.call(pl.movie, mid)) continue;
          tid++;
          var fm = pl.movie[mid];
          if (!fm || !fm.link) continue;
          var qeps = [];
          var link = String(fm.link);
          var qm = link.match(/\[(.+[\d]),?\]/i);
          if (qm) {
            var parts = qm[1].split(',');
            for (var pi = 0; pi < parts.length; pi++) {
              var qn = parseInt(parts[pi], 10);
              if (qn && qn <= maxQ) qeps.push(qn);
            }
          }
          if (!qeps.length) qeps = [720];
          var mq = Math.max.apply(null, qeps);
          var file_url = link.replace(/\[(.+[\d]),?\]/i, String(mq));
          var vname = fm.translation || fm.quality || ('Filmix ' + tid);
          extract[vname] = {
            translation: tid,
            json: { 1: { id: 1, folder: [{
              translation: tid, season: 0, episode: 0, title: vname,
              quality: mq, qualities: qeps, link: file_url, voice: vname
            }] } }
          };
        }
      }
