...
(function () {
  ...
  function statusOf(a, c) {
    try {
      if (a && a.status) return parseInt(a.status, 10) || 0;
      if (a && a.statusCode) return parseInt(a.statusCode, 10) || 0;
      if (c && c.status) return parseInt(c.status, 10) || 0;
      if (typeof a === 'string' && /500/.test(a)) return 500;
      if (typeof c === 'string' && /500/.test(c)) return 500;
    } catch (e) {}
    return 0;
  }

  function humanError(a, c) {
    var st = statusOf(a, c);
    if (st === 500) return 'Filmix: внутрішня помилка сервера (500). Спробуй пізніше, VPN або інший токен.';
    if (st === 403) return 'Filmix: доступ заборонено (403). Перевір токен / VPN.';
    if (st === 401) return 'Filmix: токен не прийнято (401). Отримай новий у кабінеті.';
    if (st === 404) return 'Filmix: не знайдено (404).';
    try {
      if (a && a.message) return 'Filmix: ' + a.message;
    } catch (e) {}
    return 'Filmix: помилка мережі' + (st ? ' (' + st + ')' : '');
  }

  function request(url, ok, fail, _try) {
    ...
    function error(a, c) {
      if (done) return;
      // один повтор на 500
      if (attempt < 1 && statusOf(a, c) === 500) {
        setTimeout(function () { request(url, ok, fail, attempt + 1); }, 800);
        return;
      }
      done = true;
      if (fail) fail(a, c);
  ...
  function FilmixSource(component, object) {
    ...
    this.search = function (obj, data) {
      ...
      net = request(url, function (json) {
        ...
        component.empty(humanError(a, c));
      });
    };
